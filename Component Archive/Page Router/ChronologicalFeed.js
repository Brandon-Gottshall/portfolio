...
function TimelineCard({ project, index }) {
  const statusClass =
    statusStyles[project.status] || statusStyles['historical artifact']
  return (
    <article className='relative grid gap-4 p-6 overflow-hidden bg-white rounded-lg nm-flat-white-lg sm:grid-cols-3'>
      ...
      <div className='pl-2'>
        ...
        <p className='pt-2 text-sm font-bold text-gray-700'>{project.lane}</p>
        <span
          className={`inline-flex px-3 py-1 mt-4 text-xs font-bold tracking-wide uppercase rounded-full font-ox ${statusClass}`}
        >
          {project.status}
        </span>
      </div>
      ...
      <div className='w-full max-w-6xl mx-auto'>
        <div className='p-6 mb-8 bg-white rounded-lg nm-flat-white-xl sm:p-10'>
          ...
          <p className='max-w-3xl pt-5 text-lg leading-8 text-gray-700'>
            This feed is the public reading order for the work: active systems,
            shipped rebuilds, focused utilities, and historical artifacts that
            still explain the direction.
          </p>
          <div className='grid gap-4 pt-8 text-sm text-gray-700 sm:grid-cols-3'>
            <div className='p-4 rounded nm-concave-white-sm'>
              <span className='block text-2xl font-black text-red-500 font-ox'>
                01
              </span>
