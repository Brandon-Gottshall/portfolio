...
import { StaleDataWarning, ZeroCommitWarning, TinyRepoWarning } from '../utils/warnings'
import { ProcessedStat } from '../types/stats'
import { shouldShowZeroCommitWarning, shouldShowRepoWarning } from '../utils/calculations'
...
function StatHeader({ stat, index, activeSegment, topItemsLength, chartColors }: StatHeaderProps) {
  return (
    <div className="flex justify-between mb-1.5 items-center">
      <div className="flex items-center">
        ...
        />
        <span className={`font-code text-navy font-medium dark:text-cream ${index >= topItemsLength ? 'text-sm' : ''}`}>
          {stat.name}
...
function StatProgress({ stat, index, activeSegment, chartColors }: StatProgressProps) {
  return (
    <Tooltip>
...
function StatItem({ stat, index, activeSegment, setActiveSegment, isDarkMode, topItemsLength, chartColors }: StatItemProps) {
  return (
    <div
    ...
    >
      <StatHeader stat={stat} index={index} activeSegment={activeSegment} topItemsLength={topItemsLength} chartColors={chartColors} />
      <StatProgress stat={stat} index={index} activeSegment={activeSegment} chartColors={chartColors} />

      <div className="flex justify-between mt-1.5 text-xs text-navy/80 dark:text-cream/80">
        <div className="flex items-center">
          <span>{stat.commits} commits</span>
          {shouldShowZeroCommitWarning(stat.commits, stat.repositories) && <ZeroCommitWarning />}
        </div>
